   ...
   RGBTRIPLE newImage[height][width];
     ...
     for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width ; j++)
        {
            ...
            if (i == 0 && j == 0)
            {
                  gx_blue += (image[i][j].rgbtBlue * 0) + (image[i + 1][j].rgbtBlue * 0) + (image[i][j - 1].rgbtBlue * -2) + (image[i][j + 1].rgbtBlue * 2) + (image[i - 1][j - 1].rgbtBlue * -1) + (image[i + 1][j - 1].rgbtBlue * -1) + (image[i - 1][j + 1].rgbtBlue * 1) + (image[i + 1][j + 1].rgbtBlue * 1)
            }
